...
**Sister sites:** `../tiedyesamurai.org` (FRC Team 10933) and `../tiedyejedi.org` (FTC, grades 6-9) are robotics-team sites — both *programs of* STEM+C, built on the same Astro/Tailwind/rsync stack. All three are tied at the hip: shared org, branding, hosting server, and architecture. When changing conventions here (image-loading pattern, brand colors, deploy flow), check whether the sibling repos should change in lockstep.
